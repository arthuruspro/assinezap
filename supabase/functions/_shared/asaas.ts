...
export async function findCustomerByCpf(cpf: string): Promise<string | null> {
  const res = await fetch(`${ASAAS_BASE}/customers?cpfCnpj=${cpf.replace(/\D/g, '')}`, {
    method: 'GET',
    headers: headers(),
  });
  const data = await res.json();
...
/** Cria autorização PIX Automático com valor e contractId customizáveis */
export async function createPixAutomaticAuthorization(
  ...
  if (!res.ok) {
    const msg = data.errors?.[0]?.description || JSON.stringify(data);
    throw new Error(`Asaas createPixAutomaticAuthorization: ${msg}`);
  }
